...
| Soft-delete edge reconnect | Edge `createdAt/deletedAt`, active-edge duplicate checks | Create edge, delete it, reconnect same relation; verify reconnect succeeds and history is preserved. Do it in the **blueprint tray** as well as on the canvas: a `feeds into` relation between two components is drawn on no other surface, so the tray is the only place it can be selected and removed. |
...
| Markdown export | Focus+Context aggregations, salience, locator codes, download | Export markdown twice and verify the bytes are identical (same machine — a re-encoded screenshot is encoder-dependent across machines); verify the most recent system screenshot appears once, above `## Abstract`, and that a project with no screenshot emits no figure, no caption and no placeholder; verify Provenance names set-aside cards; verify every cited code either resolves to a heading or renders as inert plain text, never as a literal `[R7]`; on a large project verify every live card is named, that "Also indexed" carries the ones not printed, and that each trimmed section says how many it left out. |
...
| Search/chat resilience without vectors | Embeddings table presence/error fallback path | Simulate missing/failing embeddings and confirm query/chat still return ranked results — *ranked*, not merely non-empty: chat previously fell back to document order on every non-vector path. Also confirm a card with no embedding row (created inside the debounce window, or in a freshly imported project) can still be returned, and that a question whose parsed text filter matches nothing relaxes rather than answering "no relevant nodes". |
