...
- `backend/src/utils/projectVi.ts`: the `.vi` container is **brotli**, and the byte after the `VITRALVI` magic says which codec (`1` = gzip, `2` = brotli). Version-1 files still import; only writing changed. Tunable via `VI_BROTLI_QUALITY` (default `4`) and `VI_BROTLI_WINDOW_LOG` (default `24`).
  - **The window size is the whole point, not the quality.** The bundle is one full state snapshot per revision, and consecutive snapshots are near-identical, so nearly all of a large `.vi` is repetition *between* snapshots. gzip's window is 32 KB — smaller than a single ~85 KB snapshot — so it never saw any of that and compressed each snapshot alone. Measured on a 1204-revision project: the revision section went 106 MB -> 14.2 MB under gzip -1 and 106 MB -> 0.17 MB with a 16 MB window, and the whole export went 32.7 MB -> 18.6 MB in 2.3 s -> 1.6 s. Raising `VI_BROTLI_QUALITY` buys ~1% each step and costs ~120 ms; lowering `VI_BROTLI_WINDOW_LOG` gives the size back immediately.
  - After the swap the file is ~98% asset bytes (already-compressed PNGs and PDFs, which no codec shrinks). Anything further has to come from the assets themselves — downscaling images at upload — or from storing fewer full revisions, not from the export path.
  - The export streams `jsonb::text` straight through instead of letting `pg` parse each snapshot into JS objects for `JSON.stringify` to re-print. Two passes over ~100 MB and 1204 live object graphs, gone — this is also what keeps the heap budget above honest.
  - Paged sections (revisions, embeddings, GitHub events) use **keyset** pagination, not `OFFSET`, so Postgres does not re-sort the log once per batch. Timestamp cursors must be carried as `::text`: `pg` hands a `timestamptz` over as a millisecond `Date`, and feeding that back rounds the cursor *down*, so rows sharing that millisecond repeat forever. `assertCursorAdvanced` turns that class of mistake into a failed export rather than an endless download.
