...
const FlowInnerWithProjectId = ({ projectId }: { projectId: string }) => {
    ...
    const labelFilteredNodes = useMemo(() => {
        return keepAll(timelineContextNodes, timelineContextNodes.filter((node) => {
            ...
            if (!modelDerivedVisible && (node.data as Record<string, unknown> | undefined)?.autoGenerated === true) return false;
    ...
    }, [blueprintComponentsVisible, modelDerivedVisible, selectedLabelSet, timelineContextNodes]);
    ...
    const onFreeInputSubmit = useCallback(async (x: number, y: number, userText: string) => {
        ...
        setCursorMode("");
        setLoading(true);
        // As in `onAttachFileForNode`: everything this produces is model-derived, so it must not
        // land behind the provenance filter.
        setModelDerivedVisible(true);
    ...
    }, [blueprintComponentsVisible, modelDerivedVisible, selectedLabels, queryMatchedNodeIds, fitView]);
