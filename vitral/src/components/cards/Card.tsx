...
import { faLink, faRepeat, faUserCheck, faUserPlus, faWandMagicSparkles, faXmark } from '@fortawesome/free-solid-svg-icons';
...
function CardImpl(props: CardProps) {
    ...
    const isModelDerived = isModelDerivedNodeData(props.data);
    ...
    // Everything the old citation block said, folded into one hover. Built here rather than in the
    // markup so the two halves stay in one place: the quote is what the card claims the document
    // says, the filename is which document, and either alone is still worth showing.
    const sourceTooltip = useMemo(() => {
        const parts: string[] = [];
        if (reference) parts.push(`\u201c${reference}\u201d`);
        if (originFileName) parts.push(`Source: ${originFileName}`);
        if (parts.length === 0) return "";
        if (originFile) parts.push("Click to open it here, at this passage.");
        return parts.join("\n\n");
    }, [originFile, originFileName, reference]);
            ...
            >
                ...
                <div className={`${classes.flipCardBack} ${flipping ? classes.flipCardHideBackface : ""} ${props.data.type == "social" ? classes.socialCardBack : classes.techCardBack}`}>
                    ...
                    <div className={classes.backBody}>
                        ...
                        {/*
                          * One row, pinned under the description: the note is the content of this
                          * face and everything else is a handle to it. The citation and the source
                          * filename used to sit here as two blocks of prose that between them took
                          * more of the card than the description did.
                          */}
                        <div className={classes.backFooter}>
                            {isEditingCreatedAt ? (
                                <input
                                    type="datetime-local"
                                ...
                                <select
                                    className={`${classes.assignmentSelect} nodrag`}
                                    value={assignedTo}
                                    autoFocus
                                    onChange={(event) => {
                                ...
                                <>
                                    {isActivity && !readOnly ? (
                                        ...
                                        <p className={classes.footerTimestamp}>{createdAtText}</p>
                                    )}

                                    {/*
                                      * A native `title` rather than a styled popover, and
                                      * deliberately: the card faces carry `contain: layout paint
                                      * style`, so anything drawn inside them is clipped at 200x260
                                      * -- a quote long enough to be worth reading could not be shown
                                      * there at all. The browser's own tooltip escapes the card,
                                      * wraps, and costs no layout.
                                      */}
                                    {sourceTooltip ? (
                                        originFile ? (
                                            <button
                                                type="button"
                                                className={`${classes.footerIconButton} nodrag`}
                                                title={sourceTooltip}
                                                aria-label="Show this passage in the source document"
                                                onClick={(event) => {
                                                    event.stopPropagation();
                                                    setSourceOpen(true);
                                                }}
                                            ...
                                            <span
                                                className={`${classes.footerIconButton} ${classes.footerIconInert}`}
                                                title={sourceTooltip}
                                    ...
                                    {isTaskOrRequirement && !readOnly ? (
                                        <button
                                            ...
                                            aria-label={assignedTo ? `Assigned to ${assignedTo}. Change assignment` : "Assign a participant"}
                                            onClick={(event) => {
                                                event.stopPropagation();
                                                setIsAssigning(true);
                                            }}
                                        >
                                            <FontAwesomeIcon icon={assignedTo ? faUserCheck : faUserPlus} />
                                        </button>
                                    ) : null}
                                </>
                            )}
