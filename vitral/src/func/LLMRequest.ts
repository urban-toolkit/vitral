    ...
    try {
        for (const connection of llmConnections) {
            ...
            resultingEdges.push({
                ...
                type: "relation",
                // The same marker the extracted *cards* carry, so "a model asserted this" reads the
                // same way on both halves of the graph and `RelationEdge` can badge it. Nothing
                // infers this after the fact: an edge from before the flag existed carries no
                // `manual` either, so absence has to keep meaning "unknown", not "AI".
                data: { from, to, autoGenerated: true },
            });
        }
    } catch (err) {
